...
public class DepositReverseAction implements Serializable {
    ...
    public String onConfirm() {
        try {
            String response = depService.process("1005330|" + this.msgSn);
            if (response.startsWith("0000")) { //成功
                this.confirmed = true;
                MessageUtil.addInfo("缴款冲正交易处理成功。");
            }else{
    ...
    private boolean checkMsgsn(){
        if(msgSn.length()!=18){
            ...
            if(!"5330".equals(msgSn.substring(intLength-6,intLength-2))){
                MessageUtil.addError("申请单编号不是缴款冲正编码，请检查！");
                return false;
            }
        }
        return true;
